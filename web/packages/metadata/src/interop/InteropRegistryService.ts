 ...
 */
import { distinct, ExtendedMap, flatMap, join, Logger, LoggerFactory } from '@plexus-interop/common';
...
export class InteropRegistryService {
  ...
  private appProvidedMethodsCache: ExtendedMap<string, ProvidedMethod[]> = ExtendedMap.create<
    string,
    ProvidedMethod[]
  >();
  ...
  public getApplication(idOrAlias: string): Application {
    ...
    const appFinder = (app: Application) =>
      app.id === idOrAlias || (!!app.options && !!app.options.find(aliasOptionPredicate));
  ...
  public getConsumedService(appId: string, serviceReference: ConsumedServiceReference): ConsumedService {
    ...
    const result = app.consumedServices.find(
      (consumedService) =>
        consumedService.service.id === serviceReference.serviceId &&
  ...
  public getConsumedMethod(appId: string, reference: ConsumedMethodReference): ConsumedMethod {
    ...
    const result = consumedMethods.find(
      (method) =>
        this.equalsIfExist(reference.methodId, method.method.name) &&
  ...
  public getProvidedService(reference: ProvidedServiceReference): ProvidedService {
    const result = this.getApplication(reference.applicationId as string).providedServices.find(
      (x) =>
        this.equalsIfExist(reference.serviceAlias, x.service.serviceAlias) &&
        this.equalsIfExist(reference.serviceId, x.service.id)
    );
    if (!result) {
  ...
  public getMatchingProvidedMethodsFromConsumed(consumedMethod: ConsumedMethod): ProvidedMethod[] {
    return this.getMatchingProvidedMethodsForApp(consumedMethod.consumedService.application).filter((providedMethod) =>
      this.equals(consumedMethod.method, providedMethod.method)
    );
  }
  ...
  public getProvidedMethods(): ProvidedMethod[] {
    ...
    return flatMap<ProvidedService, ProvidedMethod>((service) => service.methods.valuesArray(), allProvidedServices);
  }
  ...
  private getMatchingProvidedMethodsForAppInternal(app: Application): ProvidedMethod[] {
    const allProvidedServices: ProvidedService[] = flatMap<Application, ProvidedService>(
      (application) => application.providedServices,
      this.registry.applications.valuesArray()
    );
    ...
    const result = flatMap<{ consumed: ConsumedService; provided: ProvidedService }, ProvidedMethod>((pair) => {
      ...
      return distinct<ProvidedMethod>(
        allMatchedProvidedMethods,
