...
export class CrossDomainEventBus implements EventBus {
  ...
  private readonly stateMaschine: StateMaschine<State> = new StateMaschineBase(
    State.CREATED,
    [
  ...
  public constructor(private readonly hostIFrame: HTMLIFrameElement, private readonly hostOrigin: string) {}
  ...
  protected sharedObservable<T>(
    key: string,
    postInit?: (key: string, observable: Observable<T>) => void
  ): Observable<T> {
  ...
  private sendAndSubscribe<T, ResType>(
    ...
    return this.sharedObservable<IFrameHostMessage<T, ResType>>(subscriptionKey, () => {
      this.postToIFrame(message);
      if (message.responseType === ResponseType.Single) {
        this.rejectTimeouts.set(
          subscriptionKey,
          setTimeout(() => {
