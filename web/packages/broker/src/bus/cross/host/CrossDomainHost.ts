...
import { Logger, LoggerFactory, StateMaschine, StateMaschineBase } from '@plexus-interop/common';
...
export class CrossDomainHost {
  ...
  private stateMaschine: StateMaschine<HostState> = new StateMaschineBase<HostState>(
    HostState.CREATED,
    [
  ...
  public constructor(private readonly internalBus: EventBus, private readonly config: CrossDomainHostConfig) {
  ...
  private isLastRemoteMessage(event: Event): boolean {
    const { payload } = event;
    return (
      payload && (payload.status === RemoteActionStatus.COMPLETED || payload.status === RemoteActionStatus.FAILURE)
    );
  }
}
