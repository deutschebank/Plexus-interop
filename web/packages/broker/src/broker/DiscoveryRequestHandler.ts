...
import { ExtendedArray, Logger, LoggerFactory } from '@plexus-interop/common';
import { AppRegistryService, InteropRegistryService, ProvidedMethod } from '@plexus-interop/metadata';
import { ClientError, clientProtocol, ClientProtocolHelper, ErrorCompletion, UniqueId } from '@plexus-interop/protocol';
...
export class DiscoveryRequestHandler {
  ...
  public async handleMethodDiscovery(
    ...
    sourceConnection: ApplicationConnection
  ): Promise<void> {
    ...
    try {
      ...
      let providedMethods: ProvidedMethod[] = methodDiscoveryRequest.consumedMethod
        ? this.interopRegistryService.getMatchingProvidedMethods(
            sourceConnection.descriptor.applicationId,
      ...
      if (methodDiscoveryRequest.inputMessageId) {
        providedMethods = providedMethods.filter(
          (m) => m.method.requestMessage.id === methodDiscoveryRequest.inputMessageId
        );
      }

      if (methodDiscoveryRequest.outputMessageId) {
        providedMethods = providedMethods.filter(
          (method) => method.providedService.application.id === methodDiscoveryRequest.outputMessageId
        );
      }

      let discoveredMethods: DiscoveredMethod[];

      if (clientProtocol.interop.protocol.DiscoveryMode.Online === methodDiscoveryRequest.discoveryMode) {
