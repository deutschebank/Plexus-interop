...
import { ConsumedMethodReference, InteropRegistryService, ProvidedMethodReference } from '@plexus-interop/metadata';
import {
...
import { BufferedObserver, Defaults, PlexusObserver, TransportChannel } from '@plexus-interop/transport-common';
...
export class InvocationRequestHandler {
  ...
  public async handleRequest(
    ...
    sourceConnectionDescriptor: ApplicationConnectionDescriptor
  ): Promise<Completion> {
    ...
    try {
      const methodReference: ConsumedMethodReference | ProvidedMethodReference =
        (invocationRequest.consumedMethod as ConsumedMethodReference) || invocationRequest.providedMethod;
      ...
      try {
        ...
        targetChannel.sendMessage(
          ClientProtocolHelper.invocationRequestedPayload(
            this.createInvocationStartRequested(methodReference, sourceConnectionDescriptor)
          )
        );
  ...
  private async propogateAll(
    source: BufferedObserver<ArrayBuffer>,
    targetChannel: TransportChannel,
    sourceChannel: TransportChannel
  ): Promise<void> {
    const targetChannelId = targetChannel.uuid().toString();
    const sourceChannelId = sourceChannel.uuid().toString();
    ...
    const sourceObserver: (resolve: any, reject: any) => PlexusObserver<ArrayBuffer> = (resolve, reject) => ({
      next: async (messagePayload) => {
        if (this.log.isTraceEnabled()) {
          this.log.trace(
            `[${sourceChannelId}]->[${targetChannelId}] Transferring message of ${messagePayload.byteLength} bytes`
          );
        }
        try {
      ...
      complete: (completion) => {
        if (completion && ClientProtocolUtils.isCancelCompletion(completion)) {
          this.log.info(
            `Source channel [${sourceChannelId}] cancelled invocation, sending cancellation close to target [${targetChannelId}]`
          );
          targetChannel.close(completion);
        }
  ...
  private createInvocationStartRequested(
    methodReference: ConsumedMethodReference | ProvidedMethodReference,
    sourceConnection: ApplicationConnectionDescriptor
  ): plexus.interop.protocol.IInvocationStartRequested {
    return {
  ...
  private async resolveTargetConnection(
    methodReference: ConsumedMethodReference | ProvidedMethodReference,
    sourceConnection: ApplicationConnectionDescriptor
    ...
    if (!Types.isConsumedMethodReference(methodReference)) {
      ...
      if (methodReference.providedService && methodReference.providedService.connectionId) {
        ...
        const onlineApps = await this.appLifeCycleManager.getOnlineConnections();
        const appInstanceId = UniqueId.fromProperties(
          methodReference.providedService.applicationInstanceId as plexus.IUniqueId
        ).toString();
        ...
        if (connections.length === 0) {
          ...
          throw new Error(
            `App instance ${appInstanceId} has several connections, you need to specify ApplicationId to make call to specific connection`
          );
        } else {
        ...
        appConnection = this.appLifeCycleManager.getOrSpawnConnection(
          methodReference.providedService.applicationId,
          sourceConnection.instanceId
        );
      }
      if (!appConnection) {
    ...
    const targetMethods = this.registryService.getMatchingProvidedMethods(
      sourceConnection.applicationId,
      methodReference
    );
    if (
      targetMethods.length === 1 &&
      targetMethods[0].options !== undefined &&
      targetMethods[0].options.filter(
        (o) => o.id === 'interop.ProvidedMethodOptions.launch_on_call' && o.value === 'ALWAYS'
      ).length > 0
    ) {
      const appConnection = await this.appLifeCycleManager.spawnConnection(
        targetMethods[0].providedService.application.id
      );
      return appConnection;
    }
    const targetAppIds = targetMethods.map((method) => method.providedService.application.id);
    const appConnection = await this.appLifeCycleManager.getOrSpawnConnectionForOneOf(
      targetAppIds,
      sourceConnection.instanceId
    );
    return appConnection;
