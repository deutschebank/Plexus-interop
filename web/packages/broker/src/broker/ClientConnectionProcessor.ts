...
import { ClientError, Completion, ErrorCompletion } from '@plexus-interop/protocol';
...
export class ClientConnectionProcessor implements AsyncHandler<TransportConnection, Completion> {
        ...
        next: async (channel) => {
          ...
          if (!sourceConnection) {
            try {
              ...
              const appConnection = await this.appLifeCycleManager.acceptConnection(
                connection,
                {
            ...
            try {
              const channelCompletion = await requestsTracker.start(channelStrId, () =>
  ...
  private async completeAndDisconnect(
    connection: TransportConnection,
    requestsTracker: TasksTracker,
    log: Logger
  ): Promise<Completion> {
