...
export class PeerAppLifeCycleManager implements AppLifeCycleManager {
  ...
  public async acceptConnection(
    ...
    if ((connection as PeerProxyConnection).isProxy) {
      ...
      const connectionCacheEntry = new CacheEntry(appConnection, this.heartBitTtl, () =>
        this.handleDroppedConnection(appConnection, connectionDropped)
      );
  ...
  public async spawnConnection(applicationId: string): Promise<ApplicationConnection> {
    ...
    this.log.debug(
      `Waiting for application [${applicationId}] id with instance id [${appLaunchResponse.appInstanceId.toString()}] to connect`
    );
    try {
      await AsyncHelper.waitFor(
        () => this.isInstanceConnected(appLaunchResponse.appInstanceId),
        new CancellationToken(),
        this.heartBitPeriod,
        this.spawnConnectionTimeout
      );
      ...
      return this.getOnlineConnectionsInternal().find(
        (appConnection) => appConnection.descriptor.instanceId === launchedAppInstanceId
      ) as ApplicationConnection;
    } catch (e) {
  ...
  public async getOrSpawnConnectionForOneOf(
  ...
  private handleDroppedConnection(
    ...
    listener: (connection: ApplicationConnection) => void
  ): void {
  ...
  private isInstanceConnected(instanceId: UniqueId): boolean {
    return (
      this.getOnlineConnectionsInternal().filter(
        (appConnection) => appConnection.descriptor.instanceId.toString() === instanceId.toString()
      ).length > 0
    );
  }
