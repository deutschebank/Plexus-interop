...
export class HostTransportConnection implements TransportConnection {
  ...
  private bindToRemoteActions(): void {
    ...
    this.remoteBrokerService.host<{}, CreateChannelResponse>(
    ...
    this.remoteBrokerService.host<ChannelRequest, string>(
      RemoteActions.OPEN_CHANNEL,
      (request: ChannelRequest, responseObserver) => {
    ...
    this.remoteBrokerService.host<SendMessageRequest, {}>(
      RemoteActions.SEND_MESSAGE,
      (request: SendMessageRequest, responseObserver) =>
        new Observable((observer) => {
          if (this.log.isTraceEnabled()) {
            this.log.trace(
              `Send Message of [${request.messagePayload.length}] length for [${request.channelId}] channel received`
            );
          }
          const channel = this.getManagedChannel(request.channelId);
          if (channel) {
            channel
              .sendMessage(stringToArrayBuffer(request.messagePayload))
    ...
    this.remoteBrokerService.host<CloseChannelRequest, CloseChannelResponse>(
      RemoteActions.CLOSE_CHANNEL,
      (request: CloseChannelRequest, responseObserver) =>
        new Observable((observer) => {
          this.log.trace(`Close channel [${request.channelId}] received`);
          const channel = this.getManagedChannel(request.channelId);
          if (channel) {
            channel
              .close(request.completion)
              .then((completion) => {
