...
import { ChannelCloseFrame, ChannelOpenFrame, ConnectionCloseFrame, ConnectionOpenFrame, MessageFrame } from './model';
...
export class FramedTransportConnection implements TransportConnection, TransportFrameListener {
  ...
  public closeAndCleanUp(): void {
    ...
    this.disconnectFromSource().catch((e) => this.log.error('Failed to disconnect from source', e));
  }
  ...
  private createStateMaschine(): StateMaschine<ConnectionState> {
    return new StateMaschineBase<ConnectionState>(
      ...
      [
        ...
        // closing connection message requested
        {
          ...
          to: ConnectionState.CLOSE_REQUESTED,
        },
        ...
        {
          from: ConnectionState.CLOSE_RECEIVED,
          to: ConnectionState.CLOSED,
        },
        {
          from: ConnectionState.OPEN,
          to: ConnectionState.CLOSE_RECEIVED,
          preHandler: async () => {
        ...
        {
          from: ConnectionState.CLOSE_REQUESTED,
          to: ConnectionState.CLOSED,
        },
      ],
      this.log
  ...
  private async disconnectFromSource(): Promise<void> {
    ...
    return this.framedTransport.disconnect().then(
      () => {
  ...
  private async listenForIncomingFrames(): Promise<void> {
    ...
    this.framedTransport.open({
      next: (frame) => {
        if (this.stateMachine.isOneOf(ConnectionState.CLOSE_REQUESTED, ConnectionState.OPEN, ConnectionState.ACCEPT)) {
  ...
  private createChannelInternal(channelId: UniqueId, isIncomingChannel: boolean): ChannelDescriptor {
    ...
    const channelTransportProxy = new BufferedTransportProxy(
      this.framedTransport,
      this.connectionCancellationToken.getWriteToken(),
      proxyLogger
    );
    const dispose = async () => {
      /* istanbul ignore if */
    ...
    const channel = new FramedTransportChannel(
      channelId,
      channelTransportProxy,
      dispose,
      this.connectionCancellationToken.getWriteToken()
    );
