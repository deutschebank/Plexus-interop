...
export class InternalMessagesConverter {
  public deserialize(data: Uint8Array): Frame {
    ...
    const plainData = plexus.interop.transport.protocol.Header.toObject(
      protoFrame
    ) as plexus.interop.transport.protocol.IHeader;
  ...
  public serialize(frame: Frame): ArrayBuffer {
    return Arrays.toArrayBuffer(
      plexus.interop.transport.protocol.Header.encode(frame.internalHeaderProperties).finish()
    );
  }
}
