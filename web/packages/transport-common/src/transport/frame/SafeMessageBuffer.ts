...
import { Arrays, LimitedBufferQueue, Logger, LoggerFactory } from '@plexus-interop/common';
    ...
    if (!this.syncTimeoutId) {
      this.syncTimeoutId = globalObj.setTimeout(() => {
        try {
          ...
          this.log.info(
            `Increasing buffer with ${
              this.chunksQueue.size() < this.batchSize ? this.chunksQueue.size() : this.batchSize
            } chunks`
          );
          this.buffer = this.flushChunks(this.buffer, this.chunksQueue, this.batchSize);
          this.log.info(
            `Current buffer size - ${this.buffer.byteLength} bytes, ${this.chunksQueue.size()} chunks in queue`
          );
