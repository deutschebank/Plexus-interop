...
export class ConsumedServiceComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(
      consumedMethod$
        .pipe(
          filter(
            (state) => !!state.consumedMethod && !!state.services.interopClient
          )
        )
        .subscribe((state) => {
          ...
          this.discoveredMethods =
            state.consumedMethod.discoveredMethods.methods;
  ...
  getTitle(consumedMethod: ConsumedMethod): string {
    return `Consumed Method - ${
      consumedMethod.consumedService.service.id
    }${this.getLabel(consumedMethod.consumedService.service)}.${
  ...
  async discoverMethods() {
    const discovered = await this.interopClient.discoverAllMethods(
      this.consumedMethod
    );
    this.store.dispatch({
  ...
  payloadPreview() {
    if (
      (!this.selectedDiscoveredMethod && !this.consumedMethod) ||
      !this.interopClient
    ) {
      return "";
    }
    ...
    return this.interopClient.createPayloadPreview(
      messageId,
      this.messageContent
    );
  }
    ...
    try {
      ...
      switch (this.consumedMethod.method.type) {
        case MethodType.Unary:
          invocationClient = await this.interopClient.sendUnaryRequest(
            method,
            this.messageContent,
            handler
          );
        ...
        case MethodType.ServerStreaming:
          invocationClient =
            await this.interopClient.sendServerStreamingRequest(
              method,
              this.messageContent,
              responseObserver
            );
        ...
        case MethodType.DuplexStreaming:
          const streamingClient =
            await this.interopClient.sendBidiStreamingRequest(
              method,
              responseObserver
            );
  ...
  label(providedMethod: DiscoveredMethod): string {
    ...
    const connectionId =
      providedMethod.providedMethod.providedService.connectionId;
    const guidPostfix =
      connectionId && connectionId.hi
        ? ` - ${UniqueId.fromProperties(connectionId).toString()}`
        : "";
  ...
  createDefaultMessage() {
    ...
    this.messageContent = this.interopClient.createDefaultPayload(
      method.requestMessage.id
    );
