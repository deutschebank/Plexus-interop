...
import { ConnectionDetails, TransportType } from '../ui/AppModel';
...
export class TransportConnectionFactory {
  ...
  public async getConnectionProvider(connectionDetails: ConnectionDetails): Promise<TransportConnectionProvider> {
    return StudioExtensions.getConnectionProvider()
      .then((cp) => {
  ...
  private async lookupProviderFromConnectionDetails(
    connectionDetails: ConnectionDetails
  ): Promise<TransportConnectionProvider> {
  ...
  private createWebTransportProvider(
    appsUrl: string,
    metadataUrl: string,
    eventBusProvider: () => Promise<EventBus>
  ): TransportConnectionProvider {
    return async () => {
