...
export function retriable<T>(
  action: () => Promise<T>,
  { retriesNum, retryTimeoutInMillis, errorHandler }: RetryConfig = defaultPromiseRetryConfig
): () => Promise<T> {
  ...
  const catchHandler: CatchHandler<T> = (error) => {
    ...
    count--;
    return delayed(action, retryTimeoutInMillis).catch(catchHandler);
  };
...
export function delayed<T>(
  action: () => Promise<T>,
  timeoutInMillis: number = defaultPromiseRetryConfig.retryTimeoutInMillis
): Promise<T> {
