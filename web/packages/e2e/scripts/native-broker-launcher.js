...
function start() {
  ...
  return new Promise((resolve) => {
    ...
    onFileAdded(wsAddressDir, addressFile, (receivedPath) => {
      if (!launched) {
        launched = true;
        readClearedLine(receivedPath).then((line) =>
          resolve({
            workingDir: line,
    ...
    setTimeout(() => {
      if (!launched) {
        launched = true;
        log(
          `No file notification received, trying to read from default location ${addressFile}`
        );
        readClearedLine(addressFile).then((line) =>
          resolve({
            workingDir: line,
            process: brokerProcess,
          })
    ...
    log('Starting Broker ...');
    brokerProcess = exec(
      fullBrokerCmd + config.brokerArgs,
      {
 ...
 */
function readClearedLine(linePath) {
  return new Promise((resolve) => {
    log(`Reading first line of ${linePath}`);
    readline(linePath, (line) => {
