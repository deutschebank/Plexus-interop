...
import {
  ServerStreamingConverter,
  toGenericBidiStreamingHandler,
  toGenericStreamingHandler,
} from './streaming/converters';
import { ServerStreamingInvocationHandler } from './streaming/ServerStreamingInvocationHandler';
import { toGenericUnaryHandler, UnaryHandlerConverter } from './unary/converters';
...
export class InvocationHandlersRegistry {
  ...
  public registerServerStreamingHandler(
    ...
    this.registerServerStreamingGenericHandler(
      toGenericStreamingHandler(handler, requestType, responseType, this.marshallerProvider)
    );
  }

  public registerServerStreamingGenericHandler(
  ...
  public registerUnaryGenericHandler(handler: UnaryInvocationHandler<ArrayBuffer, ArrayBuffer>): void {
    this.registerBidiStreamingGenericHandler(
      new UnaryHandlerConverter<ArrayBuffer, ArrayBuffer>(this.log).convert(handler)
    );
  }
  ...
  public registerUnaryHandler(handler: UnaryInvocationHandler<any, any>, requestType: any, responseType: any): void {
    ...
    this.registerUnaryGenericHandler(
      toGenericUnaryHandler(handler, requestType, responseType, this.marshallerProvider)
    );
  }

  public registerBidiStreamingHandler(
    handler: BidiStreamingInvocationHandler<any, any>,
    requestType: any,
    responseType: any
  ): void {
    this.registerBidiStreamingGenericHandler(
      toGenericBidiStreamingHandler(handler, requestType, responseType, this.marshallerProvider)
    );
  }
  ...
  public getTypeAwareBidiStreamingHandler(
    actionRef: ActionReference
  ): BidiStreamingInvocationHandler<any, any> | undefined {
    return this.typeAwareHandlers.get(this.actionHash(actionRef));
  }

  public getRawBidiStreamingHandler(
    actionRef: ActionReference
  ): BidiStreamingInvocationHandler<ArrayBuffer, ArrayBuffer> | undefined {
