...
export class ServerStreamingConverter<Req, Res>
  implements InvocationHandlerConverter<ServerStreamingInvocationHandler<Req, Res>, Req, Res>
{
...
export function toGenericStreamingHandler(
  ...
  requestType: any,
  responseType: any,
  marshallerProvider: BinaryMarshallerProvider
  ...
  return {
    ...
    handle: (
      ...
      hostClient: StreamingInvocationClient<ArrayBuffer>
    ) => {
    ...
    handle: (invocationContext: MethodInvocationContext, hostClient: StreamingInvocationClient<ArrayBuffer>) => {
      ...
      return {
        next: (requestPayload: ArrayBuffer) =>
          baseObserver.next(requestMarshaller.decode(new Uint8Array(requestPayload))),
