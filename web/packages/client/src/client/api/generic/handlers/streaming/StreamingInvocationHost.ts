...
export class StreamingInvocationHost {
  ...
  public constructor(private readonly handlersRegistry: InvocationHandlersRegistry) {}

  public executeTypeAwareHandler(
  ...
  private execute(
    ...
    invocation.open({
      started: () => {
        ...
        const invocationHandler = isTypeAware
          ? this.handlersRegistry.getTypeAwareBidiStreamingHandler(actionRef)
          : this.handlersRegistry.getRawBidiStreamingHandler(actionRef);
        if (invocationHandler) {
          ...
          baseRequestObserver = invocationHandler.handle(
            invocationContext,
            new StreamingInvocationClientImpl(invocation, this.logger)
          );
        } else {
      ...
      streamCompleted: () => {
        ...
        this.handleClientAction(baseRequestObserver, () =>
          (baseRequestObserver as InvocationObserver<any>).streamCompleted()
        );
      },
      ...
      error: (invocationError) => {
        ...
        this.handleClientAction(baseRequestObserver, () =>
          (baseRequestObserver as Observer<any>).error(invocationError)
        );
      },
    });
  }
