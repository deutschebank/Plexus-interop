...
export class UnaryHandlerConverter<Req, Res>
  implements InvocationHandlerConverter<UnaryInvocationHandler<Req, Res>, Req, Res>
{
    ...
    return {
      ...
      handle: (invocationContext: MethodInvocationContext, invocationHostClient: StreamingInvocationClient<Res>) => ({
        next: (request: Req) => {
          try {
            unary
              .handle(invocationContext, request)
              .then(async (response) => {
                try {
