...
import { Arrays, Logger, LoggerFactory, onceVoid } from '@plexus-interop/common';
...
export class GenericClientApiImpl implements InternalGenericClientApi {
  ...
  public async sendUnaryRequest(
    ...
    return this.sendRawUnaryRequest(genericRequest, Arrays.toArrayBuffer(requestMarshaller.encode(request)), {
      value: (responsePayload: ArrayBuffer) =>
        responseHandler.value(responseMarshaller.decode(new Uint8Array(responsePayload))),
  ...
  public sendRawBidirectionalStreamingRequest(
  ...
  public async sendBidirectionalStreamingRequest(
    genericRequest: GenericRequest,
    responseObserver: InvocationObserver<any>,
    requestType: any,
    responseType: any
    ...
    const requestMarshaller = this.marshallerProvider.getMarshaller(requestType);
    const responseMarshaller = this.marshallerProvider.getMarshaller(responseType);
    const baseClient: StreamingInvocationClient<ArrayBuffer> = await this.sendRawBidirectionalStreamingRequest(
      genericRequest,
      {
        next: (responsePayload: ArrayBuffer) =>
          responseObserver.next(responseMarshaller.decode(new Uint8Array(responsePayload))),
  ...
  public async sendBidirectionalStreamingRequestInternal(
    strInfo: string,
    requestInvocation: () => Promise<Invocation>,
    responseObserver: InvocationObserver<ArrayBuffer>
  ): Promise<StreamingInvocationClientInternal<ArrayBuffer>> {
    ...
    await new Promise((resolve, reject) => {
      invocation.open(
        new DelegateInvocationObserver(
          new LoggingInvocationObserver(responseObserver, logger),
          (s) => resolve(s),
          (e) => reject(e)
        )
      );
  ...
  public async sendServerStreamingRequestInternal(
    ...
    streamingClient = await this.sendBidirectionalStreamingRequestInternal(
      strInfo,
      requestInvocation,
      responseObserver
    );
  ...
  public invokeUnaryHandler(
    ...
    return new InvocationExecutor(this.handlersRegistry).invokeUnaryHandler(
      invocationContext,
      actionReference,
      requestPayload
    );
  ...
  public invokeRawUnaryHandler(
    ...
    return new InvocationExecutor(this.handlersRegistry).invokeRawUnaryHandler(
      invocationContext,
      actionReference,
