...
describe('Invocation', () => {
  ...
  it('Should accept incoming invocation request and send confirmation', async () => {
    const invocationAccepted = new Promise((resolve, reject) => {
      invocation.acceptInvocation(
        new DelegateInvocationObserver(
          new LogInvocationObserver(),
          (s) => resolve(s),
          (e) => reject(e)
        )
      );
    ...
    addToInbox(
      modelHelper.invocationRequestedPayload({
  ...
  it('Should receive message and send confirmation back', async () => {
    ...
    addToInbox(
      modelHelper.messageHeaderPayload({
  ...
  async function prepareRequestedInvocation(
    observer: InvocationObserver<ArrayBuffer> = new LogInvocationObserver<ArrayBuffer>()
  ): Promise<UniqueId> {
